...
test('adds and removes topo property path assets', async () => {
	...
	document.paths.features[0].geometry.coordinates = [
		[16, 48],
		[16.001, 48.001]
	];
	const feature = document.paths.features.find(
		(item) => String(item.id) === String(route.pathRefs[0].pathId)
	);
