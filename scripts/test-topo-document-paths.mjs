...
const legacy = {
	routes: [
		{ id: 'route-1', assets: { paths: [{ role: 'approach', label: 'Common', path: { type: 'LineString', coordinates: [[1, 2], [3, 4]] } }], keep: true } },
		{ id: 'route-2', pathRefs: [] }
	]
};
...
const shared = normalizeTopoPaths({
	paths: { type: 'FeatureCollection', features: [{ type: 'Feature', id: 'shared', properties: {}, geometry: { type: 'LineString', coordinates: [[0, 0], [1, 1], [2, 2]] } }] },
	routes: [{ id: 'a', pathRefs: [{ pathId: 'shared', role: 'approach' }] }, { id: 'b', pathRefs: [{ pathId: 'shared', role: 'descent' }] }]
}).data;
assert.deepEqual(splitTopoPath(shared, 'shared', [[0, 0], [1, 1]], [[1, 1], [2, 2]]), ['shared', 'shared-2']);
...
const routeSpecific = normalizeTopoPaths({
	paths: { type: 'FeatureCollection', features: [{ type: 'Feature', id: 'shared', properties: {}, geometry: { type: 'LineString', coordinates: [[0, 0], [1, 1], [2, 2]] } }] },
	routes: [{ id: 'a', pathRefs: [{ pathId: 'shared' }] }, { id: 'b', pathRefs: [{ pathId: 'shared' }] }]
}).data;
splitTopoPath(routeSpecific, 'shared', [[0, 0], [1, 1]], [[1, 1], [2, 2]], { mode: 'route-specific', routeId: 'a' });
assert.equal(routeSpecific.paths.features.length, 3, 'route-specific split preserves the shared source path');
assert.equal(routeSpecific.routes[0].pathRefs.length, 2);
assert.deepEqual(routeSpecific.routes[1].pathRefs.map((ref) => ref.pathId), ['shared']);
assert.deepEqual(validateTopoPaths(routeSpecific), []);

const invalid = normalizeTopoPaths({ routes: [{ id: 'broken', pathRefs: [{ pathId: 'missing' }] }] }).data;
assert.match(validateTopoPaths(invalid).join('\n'), /missing/);
console.log('topo document path helpers: valid');
