		...
		<div class="panel p-1.5 flex items-center justify-between shadow-panel bg-white">
			<div class="flex items-center gap-2.5">
				...
				<div class="flex items-center gap-1">
					{#if workspace.includes('3d') && (workspace.includes('create') || userState.clustering.rawHits.length > 0)}
						<button
							...
							onclick={() => { activeTool = 'ai-bolts'; drawingTarget = null; }} title={$_('ui.ai-bolts')}>
					...
					<button
						...
						onclick={() => { activeTool = 'route'; drawingTarget = null; }} title={$_('ui.route')}>
					...
					<button
						...
						onclick={() => { activeTool = 'multipitch'; drawingTarget = null; }} title={$_('ui.multipitch')}>
					...
					<button
						...
						onclick={() => { activeTool = 'fixpoint'; drawingTarget = null; }} title={$_('ui.fixpoint')}>
					...
					<button
						...
						onclick={() => { activeTool = 'crop'; drawingTarget = null; lassoPoints = []; }} title={$_('ui.crop')}>
				...
				<label
					...
					<input type="checkbox" bind:checked={userState.clustering.showCameraTrail}
								 class="w-3 h-3 rounded-sm border border-black/15 text-creator-blue focus:ring-1 focus:ring-creator-blue transition-none" />
					<span
						class="text-ui-label text-warm-gray-500 group-hover:text-near-black transition-none">{$_('ui.show_camera_trail')}</span>
				</label>
			</div>
...
{#if userState.topo.editorMode === '3d' && activeTool && activeTool !== 'ai-bolts'}
	<div
		...
		<div class="flex items-center gap-4 px-1 text-warm-gray-500">
			{#if activeTool === 'route'}
				<div class="flex items-center gap-1.5 text-micro-data"><span>{$_('ui.set_vertex')}</span> <kbd
					class="px-1.5 py-0.5 bg-black/5 border border-black/15 rounded-sm text-[9px] font-mono text-near-black font-bold shadow-sm">Dbl
					Click</kbd></div>
				<div class="flex items-center gap-1.5 text-micro-data"><span>{$_('ui.undo_vertex')}</span> <kbd
					class="px-1.5 py-0.5 bg-black/5 border border-black/15 rounded-sm text-[9px] font-mono text-near-black font-bold shadow-sm">Backspace</kbd>
				</div>
				<div class="flex items-center gap-1.5 text-micro-data"><span>{$_('ui.finalize')}</span> <kbd
					class="px-1.5 py-0.5 bg-black/5 border border-black/15 rounded-sm text-[9px] font-mono text-near-black font-bold shadow-sm">Enter</kbd>
				</div>
			{:else if activeTool === 'multipitch'}
				<div class="flex items-center gap-1.5 text-micro-data"><span>{$_('ui.vertex')}</span> <kbd
					class="px-1.5 py-0.5 bg-black/5 border border-black/15 rounded-sm text-[9px] font-mono text-near-black font-bold shadow-sm">Dbl
					Click</kbd></div>
				<div class="flex items-center gap-1.5 text-micro-data"><span>{$_('ui.undo_vertex')}</span> <kbd
					class="px-1.5 py-0.5 bg-black/5 border border-black/15 rounded-sm text-[9px] font-mono text-near-black font-bold shadow-sm">Backspace</kbd>
				</div>
				<div class="flex items-center gap-1.5 text-micro-data"><span>{$_('ui.place_belay')}</span> <kbd
					class="px-1.5 py-0.5 bg-black/5 border border-black/15 rounded-sm text-[9px] font-mono text-near-black font-bold shadow-sm">B</kbd>
				</div>
				<div class="flex items-center gap-1.5 text-micro-data"><span>{$_('ui.finalize')}</span> <kbd
					class="px-1.5 py-0.5 bg-black/5 border border-black/15 rounded-sm text-[9px] font-mono text-near-black font-bold shadow-sm">Enter</kbd>
				</div>
			{:else if activeTool === 'fixpoint'}
				<div class="flex items-center gap-1.5 text-micro-data"><span>{$_('ui.place_point')}</span> <kbd
					class="px-1.5 py-0.5 bg-black/5 border border-black/15 rounded-sm text-[9px] font-mono text-near-black font-bold shadow-sm">Dbl
					Click</kbd></div>
				...
				<div class="flex items-center gap-1">
					{#each ['bolt', 'belay', 'piton', 'tree', 'abseil'] as type}
						...
						>
							<img src="{base}/icons/topo-symbols/{type}.svg" alt={type}
									 class="w-3 h-3 {userState.ui.selectedSymbol === type ? 'invert brightness-0' : 'opacity-70'}" />
							<span class="text-[9px] font-bold uppercase tracking-tighter">{$_(`topo.fixpoints.${type}`)}</span>
						</button>
					{/each}
				</div>
			{:else if activeTool === 'crop'}
				<div class="flex items-center gap-1.5 text-micro-data"><span>Select Area</span> <kbd
					class="px-1.5 py-0.5 bg-black/5 border border-black/15 rounded-sm text-[9px] font-mono text-near-black font-bold shadow-sm">Shift
					+ Drag</kbd></div>
				<div class="flex items-center gap-1.5 text-micro-data"><span>Select Islands</span> <kbd
					class="px-1.5 py-0.5 bg-black/5 border border-black/15 rounded-sm text-[9px] font-mono text-near-black font-bold shadow-sm">C</kbd>
				</div>
				<div class="flex items-center gap-1.5 text-micro-data"><span>Apply Cut</span> <kbd
					class="px-1.5 py-0.5 bg-black/5 border border-black/15 rounded-sm text-[9px] font-mono text-near-black font-bold shadow-sm">Delete</kbd>
				</div>
				<div class="flex items-center gap-1.5 text-micro-data"><span>Reset</span> <kbd
					class="px-1.5 py-0.5 bg-black/5 border border-black/15 rounded-sm text-[9px] font-mono text-near-black font-bold shadow-sm">Esc</kbd>
				</div>
			{/if}
		</div>
	</div>
...
<div
	class="fixed {activeTool && activeTool !== 'ai-bolts' ? 'top-[100px]' : 'top-[56px]'} left-2 z-40 flex flex-col w-80 max-h-[calc(100vh-8rem)] overflow-y-auto custom-scrollbar">
...
<div class="h-screen w-screen absolute overflow-hidden bg-warm-white">
	{#if userState.topo.editorMode === '2d'}
		<Topo2DEditor bind:this={editor2D} bind:activeTool selectedSymbol={userState.ui.selectedSymbol} {drawingTarget}
	...
	{:else}
		...
		<Canvas linear {createRenderer} dpr={browser ? window.devicePixelRatio : 1}>
			<T.PerspectiveCamera makeDefault position={cameraPosition} fov={75} near={0.1} far={1000}>
				<OrbitControls bind:ref={controlsRef} enableZoom={true} target={controlsTarget}
			...
			{#if element !== undefined}
				<Model bind:this={modelComponent} gltfScene={loadedGltfScene} position={modelPositionOffset}
							 scale={userState.topo.scale} {activeTool} {drawingTarget} {element} bind:cameraPosition
							 bind:controlsTarget bind:selectedIndicesMap>
					<EditorInternal bind:this={editorInternal} {loadedGltfScene} {element} bind:selectedIndicesMap {activeTool}
													{drawingTarget} {cameraPosition} {controlsTarget} {controlsRef} />
				</Model>
			{/if}
		</Canvas>
	{/if}
