...
export class Topo3DInteractionManager {
	...
	getSnappedVertex(point) {
		...
		let found = null; // { point, id, type, routeId, index }
	...
	handleFixPointDblClick(e, pointId, activeTool) {
		...
		if (
			...
			const segmentPoints = this.generateProjectedSegment(
				lastClick,
				snapClick,
				this.gltfScene,
				e.camera
			...
			if (activeTool === 'multipitch') {
				...
				if (route) {
					let pitch = this.localDrawingState
						? route.pitches.find((p) => p.id === this.localDrawingState.pitchId)
						: null;
